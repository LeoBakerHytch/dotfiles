...
fun <SID>ToggleAutoCloseMappings() " --- {{{2
    if g:autoclose_on
        ...
        inoremap <silent> } <C-R>=<SID>CloseStackPop('}')<CR>
        inoremap <silent> <CR> <C-R>=<SID>OpenBlock('{', '}')<CR>
        ...
        inoremap <silent> <C-C> <C-R>=<SID>CloseStackPop('')<CR><C-C>
...
function <SID>OpenBlock(ochar,cchar)
    let line = getline('.')
    let col = col('.') - 2
    if a:ochar == line[(col)] && a:cchar == line[(col+1)]
        while len(s:closeStack) > 0
            call remove(s:closeStack, 0)
        endwhile
        return "\<esc>a\<CR>\<esc>O"
